...
import Queue from "../3_Queue/2_priority_queue_1";
import LinkedStack from "../2_Stack/1_stack_basics";
import PriorityQueue from "../3_Queue/2_priority_queue_1";
...
export default class Graph {
  ...
  addEdge(vertexOne, vertexTwo, weight) {
    throw new Error(
      "You must implement your own addEdge method for undirected or directed graph."
    );
  }

  removeEdge(vertexOne, vertexTwo) {
    throw new Error(
      "You must implement your own removeEdge method for undirected or directed graph."
    );
  }
    ...
    while (!priorityQueue.isEmpty()) {
      ...
      for (const linkedVertex of this.adjacencyList[vertex]) {
        if (!visited[linkedVertex.value]) {
          ...
          if (
            !prev[linkedVertex.value] ||
            prev[linkedVertex.value][1] > newWeight
          ) {
  ...
  buildAdjacencyMatrix(vertices = Object.keys(this.adjacencyList)) {
    ...
    const matrix = Array.from({ length: vertices.length }, (row, i) =>
      Array.from({ length: vertices.length }, (elem, j) =>
        i === j ? 0 : Infinity
      )
    );
  ...
  travelingSalesmanProblemBF(vertex) {
    ...
    function findPaths(vertex, visited = {}, path = []) {
      ...
      if (
        ...
        self.adjacencyList[vertex].some(
          (linkedVertex) => linkedVertex.value === start
        )
      ) {
    ...
    for (const path of paths) {
      ...
      for (let i = 0; i < path.length - 1; i++) {
        currentDistance +=
          matrix[verticesObj[path[i]]][verticesObj[path[i + 1]]];
      }
  ...
  travelingSalesmanProblemDP(vertex) {
    ...
    function findPath(state, position) {
      ...
      for (
        let indexOfVertex = 0;
        indexOfVertex < numberOfVertices;
        indexOfVertex++
      ) {
