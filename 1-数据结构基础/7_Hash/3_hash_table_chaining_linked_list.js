...
import SinglyLinkedList from "../4_LinkedList/2_singly_linked_list";
...
class HashTable {
  ...
  delete(key) {
    const bucketIndex = this.hash(key);
    const itemIndex = this.keyMap[bucketIndex].find(
      (data) => data[0] === key,
      true
    );
  ...
  print() {
    for (let i = 0; i < this.keyMap.length; i++) {
      if (this.keyMap[i].head)
        console.log(`bucket ${i}: ${this.keyMap[i].iterate()}`);
    }
  }
}
